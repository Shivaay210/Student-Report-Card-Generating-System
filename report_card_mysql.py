...
def add_student():
    conn = mysql.connector.connect(
        host='localhost', database='report_card', user='root', password='root',port='3307')
    cursor = conn.cursor()
    clear()
...
def report_single_term(): 
    ...
    t = PrettyTable(['Subject',' Max_marks','min-marks','marks obtained'])
    t.add_row(['Physics', '100', '33', record[3]])
    t.add_row(['Chemistry', '100', '33', record[5]])
    t.add_row(['Mathematics', '100', '33', record[4]])
    t.add_row(['English', '100', '33', record[6]])
    t.add_row(['Computer', '100', '33', record[7]])
    print(t)
...
def report_topper_list():
    ...
    print('T O P P E R S    L I S T \n\n Class :',clas,'      Section :',section,'     Session : ', session, ' Term :',term)
    #print('-'*120)
